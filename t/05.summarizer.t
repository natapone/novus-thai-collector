...
subtest 'Extract key phrase' => sub {
    ...
    foreach my $ngram ( 
        ...
        ok(defined($keywords->{$ngram}->{'SCORE'}), "Summarize result has SCORE" );
        ok(defined($keywords->{$ngram}->{'TFIDF'}), "Summarize result has TFIDF" );
        ok(defined($keywords->{$ngram}->{'PROB'}), "Summarize result has PROB" );
        ok(defined($keywords->{$ngram}->{'key_length'}), "Summarize result has key_length" );
